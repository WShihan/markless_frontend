<template>
  <div v-if="link" class="link">
    ...
    <div class="link-opts">
      <a v-if="link.read" @click.prevent="unreadLink">
        <svg
          t="1726443111970"
          class="icon"
...
import { linkDel } from '@/api';
import { linkRead } from '@/api';
...
function deleteLink(evt) {
  linkDel({ id: props.link.id }).then(res => {
...
function readLink() {
  linkRead({ id: props.link.id }).then(res => {
    const { status } = res.data;
    if (status) {
      emit('read', props.link);
